...
3. **Save**: Save this JSON inside the workspace as:
   `D:\MARKET_INTEL\scripts\social_engine\raw_cookies.json`
4. **Convert to Playwright Format**: Run the conversion script to format them for Playwright:
   ```bash
   python scripts/social_engine/convert_cookies.py
   ```
   *(Verify that the file `D:\MARKET_INTEL\scripts\social_engine\twitter_cookies.json` was generated)*
5. **Test the Pipeline Manually**: Open a terminal in the root workspace `D:\MARKET_INTEL` and run:
   ```bash
   python scripts/social_engine/fetch_tweets.py | python scripts/social_engine/save_tweets.py
   ```
...
3. **Review the Nodes**:
   ...
   * **Execute Ingestion Pipeline**: Runs the command:
     `python "D:\MARKET_INTEL\scripts\social_engine\fetch_tweets.py" | python "D:\MARKET_INTEL\scripts\social_engine\save_tweets.py"`
